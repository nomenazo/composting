    ...
    def resolution(self):
        def system(t, Y):
          ...
          #  flT2 = np.exp(logsumexp(((self.T - 338.15) *((self.T - 303.95)*(self.T - 303.95))))/\
            ...
            v13 = 0 #self.k['µmb']*self.Xmb  *self.fSc #*self.fSn  *self.flT1*self.faB_C
            v14 = 0 #self.k['µmb']*self.Xmb  * self.fSp  # *self.flT1
            v15 = 0 #self.k['µmb']*self.Xmb  *self.fSl  # *self.fSn  *self.flT1 *self.faB_L
            #2-2-Thermophilic bacterias growth
            v16 = 0 #self.k['µtb']*self.Xtb  * self.fSc  # *self.fSn   *self.flT2 *self.faB_C 
            v17 = 0 #self.k['µtb']*self.Xtb *self.fSp   #*self.flT2
            v18 = 0 #self.k['µtb']*self.Xtb *self.fSl  #*self.fSn    *self.flT2  *self.faB_L
        ...
        self.solution = solve_ivp(system, [0, self.duration], [self.va['G'],self.va['P'],self.va['L'],                                                self.va['HE'],self.va['CE'],self.va['LG'], self.va['Xi'],                                                    0,0,0,0,0,                                                     self.MB,self.TB,self.MA,self.TA,self.MF,                                                     self.TF,self.Xa, self.va['Xdb'], 0,                                                     0,0,0,0,0,0, self.va['W'], 0.000], t_eval = np.arange(0,self.duration,1), method='RK45') #, t_eval = np.arange(0,self.duration,1)
...
essai1 = Compostage(Data3, Data4, 'test_fitting', 'Passive aeration', 1000, 1e-1, 1e-1, 1e-1,1e-1,1e-1,1e-1,1e-1,0)
...
essai7 = Compostage(Data3, Data4, 'test_fitting', 'Passive aeration', 75, 1e-7, 1e-7, 1e-7,1e-7,1e-7,1e-7,1e-7,0)
...
essai3 = Compostage(Data3, Data5, 'test_fitting', 'Passive aeration', 2000, 0.001, 0.001, 0.001,0.001,0.001,0.001,0.001,0)
...
plt.plot(essai3.solution.t,essai3.solution.y[10])
