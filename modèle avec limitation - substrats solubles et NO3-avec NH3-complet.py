...
# - Dans ce code, toutes les réactions sont présentes, sauf celle qui produit le CH4 
...
class Compostage:
    def __init__(self, data_path, data_T, technology, aer, duration, MB, TB, MA, TA, MF, TF, Xa, xtemp):
        ...
        with pd.ExcelFile(data_T) as e:
            ...
            self.Tint = np.array(self.Ti[0]) #self.xtemp +
    ...
    def resolution(self):
        ...
        dt = 1
        self.solution = solve_ivp(system, [0, self.duration], [self.va['G'],self.va['P'],self.va['L'],                                                self.va['HE'],self.va['CE'],self.va['LG'], self.va['Xi'],                                                    0,0,0,0,0,                                                     self.MB,self.TB,self.MA,self.TA,self.MF,                                                     self.TF,self.Xa, self.va['Xdb'], 0,                                                     0,0,0,0,0,0, self.va['W'], 0.000,0], t_eval = np.arange(0,self.duration,dt),method='RK45')
    ...
    def courbes(self):
        ...
        functions = [self.solution.y[0], self.solution.y[7], self.solution.y[12], self.solution.y[13], self.solution.y[19],                     self.solution.y[20]]
...
essai2 = Compostage(Data3, Data4, 'test_fitting', 'Passive aeration', 1000, 1e-2, 1e-2, 1e-2,1e-2,1e-2,1e-2,1e-2,0)
...
np.shape.essai2.solution.t


# Le calcul pour une biomasse 1e-2 est lent ...

# In[370]:


plt.plot(essai2.solution.t,essai2.solution.y[16])


# In[376]:


plt.plot(essai2.solution.t,essai2.solution.y[23])
...
# - Sc est consommé très rapidement par rapport à l'hydrolyse de C
# - Sp part au négatif rapidement, puis revient au positif et devient quasiment constant. Est-ce dû aux calculs maths, ou au recyclage de P?
# - Sl est consommé très rapidement par rapport à l'hydrolyse de L dans les premiers instants (ça part au négatif) puis revient au positif,
# - toutes les biomasses décroissent (sans connaitre une croissance)
# 

# In[377]:


essai2.solution.y[20][-1]


# In[46]:


essai7 = Compostage(Data3, Data4, 'test_fitting', 'Passive aeration', 480, 1e-7, 1e-7, 1e-7,1e-7,1e-7,1e-7,1e-7,0)
...
essai3 = Compostage(Data3, Data5, 'test_fitting', 'Passive aeration',480, 0.001, 0.001, 0.001,0.001,0.001,0.001,0.001,0)
...
plt.plot(essai3.solution.t,essai3.solution.y[20])


# In[71]:


essai3.solution.y[20][-1]
