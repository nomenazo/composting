function dydt=compostfitted(t,y); %here kh are fitted with fitcompostcumulv2
...
%kh = [0.04 0.02 0.01 0.02 0.04 0.01 0.009 0.007 0.007 0.009 0.007 0.007]; %kh not fitted
%kh = [0.0789    0.0735    0.1350    0.0037    0.0393    0.0486    0.0090    0.0002    0.0197    0.0090    0.0001    0.0199]; %kh retrouvés lorsque l'on a ajouté variables O2 et CH4 au fitting
%kh = [0.0396    0.0199    0.0110    0.0200    0.0400    0.0104    0.0090    0.0070    0.0070    0.0090    0.0069    0.0072];
%kh=[2.2908e-06    0.0223    0.1790    0.0059    0.0267    0.0408    0.0090    0.0139    0.0303    0.0090    0.0140    0.0293];
kh=[0.0293    0.1508    1.4563e-07    0.0053    0.1731    0.0182    0.0090    0.0070    0.0068    0.0090    0.0070    0.0068]; %dernier essai lorsque t=[0,2280]
mu = [0.2 0.18 0.1 0.12 0.1 0.1]; %specific growth rate (h-1)
bd = [0.03 0.02 0.01 0.015 0.01 0.01]; %death rate (h-1)
K=[6.2e-5 1e-4 0.2 0.0025 0.0064]; %kinetic parameters
...
ntot = (1/(1-xO2))*((CO2/0.044)+(CH4/0.016))*TM;  %mol
Pio2 = (xO2*ntot*R*T)/Vgas; %Pa
O2diss = khO2 * Pio2 * (0.032*1e-3); %kg/l

fO2 = O2diss/(kO2+ O2diss);
...
%dO2liqdt = khO2*(0.21/22.4)*(Qair/rhoair)*R*T/Vgas; %mol/m3
%dO2dissdt=(khO2*(0.79/22.4)*(Qair/rhoair)*R*T/Vgas)*W*1e-3*32e-3
%dO2dissdt = (khO2*(0.21/22.4)*(Qair/rhoair)*R*T/Vgas)*W*1e-3*32e-3-((Ymb_c_o2)*v13+(Ymb_p_o2)*v14+(Ymb_l_o2)*v15+(Ytb_c_o2)*v16+(Ytb_p_o2)*v17+(Ytb_l_o2)*v18+(Yma_c_o2)*v19+(Yma_p_o2)*v20+(Yma_l_o2)*v21+...
%(Yma_h_o2)*v22+(Yta_c_o2)*v23+(Yta_p_o2)*v24+(Yta_l_o2)*v25+(Yta_h_o2)*v26+(Ymf_c_o2)*v27+(Ymf_p_o2)*v28+(Ymf_l_o2)*v29+...
...
%Methane module
dCH4gendt= (Ych4_Sc*(v1+v4+v8+v11 )+Ych4_Sp*(v2+v5 )+Ych4_Sl*(v3+v6 )+Ych4_Sh* (v7+v10 )+Ych4_Slg*(v9+v12 ))*(1/(1+eta*(O2diss/(32e-3)))); %kg/kgTM.h


dCH4oxidt = Vmax*((CH4gen/W)/(km+(CH4gen/W)))*((O2diss/(32e-3))/(Kch4_O2+(O2diss/(32e-3))));
...
dydt = [dCdt,dPdt,dLdt,dHdt,dCEdt,dLGdt,dXidt, dScdt,dSpdt,dSldt,dShdt,dSlgdt, dXmbdt,dXtbdt,dXmadt,dXtadt,dXmfdt,dXtfdt, dXdbdt, dCO2dt, dWdt,dTdt,dCH4gendt, dCH4oxidt,dCH4dt]'; %,dO2transdt,dO2indt]' %]';