function dydt=windrow_vf5(t,y); %we check stochiometry, Xi and denitrification
...
mNH3 = ((0.017/TM) * pNH3 * (Vgas/(R*T))); %NH3 in the gas phase
...
v47 = Qair * mNH3 /((CO2+CH4+N2+mNH3+N2O+O2gaz)*TM);
...
%%calcul de la masse totale de gaz
ntot = ((CO2/0.044)+(CH4/0.016)+(mNH3/0.017)+(N2O/0.044)+(N2/0.028)+(Wvap/0.018)+(O2gaz/0.032))*TM;  %mol


%Global equations
dCdt= -v1 -v4;
dPdt =   -v5 -v2 + 0.6853*(v43+v46);
dLdt = -v3-v6;
dHdt= -v7-v10;
dCEdt = -v8-v11;
dLGdt= -v9-v12;
...
%Ps = 10^(8.07131 -(1730.63/(T-275+233.426)));
%Ps = ((T-275)/100)^4;
%Ps= exp(23.1964-(3816.44/(T-46.13)));
%Ps = exp(13.7-(5120/T));
%Ps = exp(11.961 -(3993.7/(T+233.9)));

%dWvapdt = (klh2O/TM) * (Ps - Pwvap) - Qair*Wvap/(rhoair *Vgas*TM);


dWvapdt = (Qair)*((0.1158-(0.0072*(T-273))+(1e-4*((T-273)^2)))+...
    ((0.1158-(0.0072*(Ta-273))+(1e-4*((Ta-273)^2)))));
...
dWdt =(((Ymb_c_h)*v13+(Ymb_p_h)*v14+(Ymb_l_h)*v15+(Ytb_c_h)*v16+(Ytb_p_h)*v17+(Ytb_l_h)*v18+(Yma_c_h)*v19+(Yma_p_h)*v20+(Yma_l_h)*v21+...
    ...
    (Ymf_h_h)*v30+(Ymf_lg_h)*v31+(Ytf_c_h)*v32+(Ytf_p_h)*v33+(Ytf_l_h)*v34+(Ytf_h_h)*v35+(Ytf_lg_h)*v36)) - (Qair)*((0.1158-(0.0072*(T-273))+(1e-4*((T-273)^2)))+...
    ((0.1158-(0.0072*(Ta-273))+(1e-4*((Ta-273)^2)))))+0.842*v45 + 2.25*Vmax*((CH4gen/W)/(km+(CH4gen/W)))*((O2diss/(W*32e-3))/(Kch4_O2+(O2diss/(W*32e-3)))); %Petric 2005
...
dO2gazdt= Qair*0.23/TM - klO2*(O2eq-O2diss) - (Qair*O2gaz)/((CO2+CH4+N2+mNH3+N2O+O2gaz)*TM*rhoair); %0.23 is* the mass fraction of O2 in the air, 

dO2outdt = (Qair*O2gaz)/((CO2+CH4+N2+mNH3+N2O+O2gaz)*TM*rhoair);

dairdt = Qair*0.23/TM;

%dO2gazdt= (Qair/rhoair)*((210*0.032)/(22.4*TM)) - klO2*(O2eq-O2diss) - (((Qair/rhoair)*O2gaz)/Vgas); %0.23 is* the mass fraction of O2 in the air, 
...
dydt = [dCdt,dPdt,dLdt,dHdt,dCEdt,dLGdt,dXidt, dScdt,dSpdt,dSldt,dShdt,dSlgdt, dXmbdt,dXtbdt,dXmadt,dXtadt,dXmfdt,dXtfdt, dXdbdt, dCO2dt, dWdt,dTdt,dCH4gendt, dCH4oxidt,dCH4dt,...
    dXadt, dNO3dt, dN2Odt, dN2dt, dNH3dt, dNH4dt,dWvapdt,dO2dissdt,dO2gazdt,dO2consdt,dO2outdt,dairdt]'; %,dO2transdt,dO2indt]' %]';